    ...
    <div className="modal-backdrop" onClick={onClose}>
      ...
      >
        <div className="relative h-52 rounded-t-3xl overflow-hidden">
          ...
          <button onClick={onClose} className="absolute top-4 right-4 w-8 h-8 rounded-full glass border border-border/60 flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors">
            <HiX size={15} />
          </button>
        </div>
        ...
        <div className="p-6 flex flex-col gap-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              { label: "Frontend", items: project.tech.frontend },
              { label: "Backend", items: project.tech.backend },
            ].map(({ label, items }) => (
              <div key={label}>
                <h4 className="font-mono text-[10px] text-muted-foreground uppercase tracking-widest mb-2">{label}</h4>
                <div className="flex flex-wrap gap-1.5">
                  {items.map((t) => (
                    <span key={t} className="font-mono text-[10px] px-2 py-0.5 rounded-md bg-secondary/80 text-muted-foreground">{t}</span>
                  ))}
                </div>
              </div>
            ))}
          ...
          <div className="flex gap-3 pt-1">
            <a href={project.github} target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-2 px-4 py-2 rounded-xl border border-border text-muted-foreground hover:text-foreground hover:border-foreground/30 transition-all text-xs font-mono">
              <SiGithub size={14} /> GitHub
            </a>
            {project.live && (
              <a href={project.live} target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-2 px-4 py-2 rounded-xl text-white text-xs font-mono transition-opacity hover:opacity-90"
...
function ProjectCard({ project, index, onClick }: { project: Project; index: number; onClick: () => void }) {
  return (
    <div
      className={`project-card-${index} glass-card rounded-2xl overflow-hidden cursor-pointer group`}
      onClick={onClick}
    ...
    >
      <div className="relative h-52 overflow-hidden">
        ...
        <div className="absolute top-4 right-4 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
          <a href={project.github} target="_blank" rel="noopener noreferrer"
            onClick={(e) => e.stopPropagation()}
            className="w-7 h-7 rounded-full glass border border-border/60 flex items-center justify-center text-muted-foreground hover:text-foreground">
            <SiGithub size={12} />
          </a>
          {project.live && (
            <a href={project.live} target="_blank" rel="noopener noreferrer"
      ...
      <div className="p-5 flex flex-col gap-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          {project.tech.frontend.length + project.tech.backend.length > 4 && (
            <span className="font-mono text-[10px] px-2 py-0.5 rounded-md bg-secondary/80 text-muted-foreground">
              +{project.tech.frontend.length + project.tech.backend.length - 4}
            </span>
          )}
        </div>
    ...
    const ctx = gsap.context(() => {
      ...
      projects.forEach((_, i) => {
        gsap.to(`.project-card-${i}`, {
          y: 0, opacity: 1, duration: 0.8, ease: "power3.out",
          scrollTrigger: { trigger: `.project-card-${i}`, start: "top 85%", once: true },
          delay: i * 0.1,
        });
      });
    }, sectionRef);
      ...
      <div className="section-container">
        <div className="proj-header text-center mb-16">
          <div className="section-label justify-center mb-3">
            <span className="font-mono text-xs text-primary tracking-[0.2em] uppercase">Work</span>
          </div>
        ...
        <div className="text-center mt-12">
          <a
            ...
            className="inline-flex items-center gap-2 px-6 py-2.5 border border-border rounded-full font-mono text-xs text-muted-foreground hover:text-foreground hover:border-foreground/30 transition-all"
          >
            <SiGithub size={14} /> More on GitHub
          </a>
        </div>
      </div>
